...
try {
 ...
 await page.goto('http://127.0.0.1:8082/@fs/'+resolve('scripts/slam-mapping/route.html'))
 await page.waitForFunction(()=>window.__routeReview||window.__routeError,{},{timeout:240000})
 const report=await page.evaluate(()=>window.__routeReview??{error:window.__routeError})
 await writeFile('docs/slam-demo/phase5/route.json',JSON.stringify({browser:browser.version(),...report},null,2)+'\n')
