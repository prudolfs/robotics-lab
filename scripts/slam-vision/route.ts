...
let simulation=setStatus(createSimulation({noise:false}),'running'),last=-1
const process=(frame:StereoFrame)=>new Promise<VisionFrame>((resolve,reject)=>{worker.onmessage=({data})=>data.error?reject(Error(data.error)):resolve(data);worker.onerror=e=>reject(Error(e.message));worker.postMessage(frame,[frame.left,frame.right])})
...
try {
 while(simulation.status==='running'){
  if(simulation.tick%6===0&&simulation.tick!==last){
   ...
   const result=await process({generation:0,frameId,timestamp:frameId/10,calibration:manifest.calibration,kind:'rendered',left,right})
   evaluate.accept(result.vo,simulation.truth.pose)
   const metric=evaluate.snapshot(),v=result.vo
   measurements.push({frameId,timestamp:frameId/10,status:v?.status,reason:v?.reason,stereo:v?.stereo,matches:v?.matches,inliers:v?.inliers,rmse:v?.rmse,positionError:metric.error,trajectoryRms:metric.rms,pose:v?.pose,truth:{...simulation.truth.pose},nativeObjects:v?.nativeObjects,peakNativeObjects:v?.peakNativeObjects,processingMs:result.processingMs})
 ...
 Object.assign(window,{__routeReview:{measurements,wallMs:performance.now()-start,evaluation:evaluate.snapshot()}})
