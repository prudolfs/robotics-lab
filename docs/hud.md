...
### ✅ Phase 5 — Utils tab content (logs) + polish gaps  **[DONE]**
...
- `apps/simulator/src/lib/logger.ts` (new) — the ring-buffer logger (option 1, kept <+90 lines incl. comments / types, the runtime logic is ~55 lines). Plain module state, no sim-loop ownership, no Zustand slice. A capped (`MAX_LOGS = 200`) `entries: LogEntry[]` is the buffer; `useSyncExternalStore` is the React binding so the widget re-renders on every new line with no extra subscription plumbing. `emit` replaces (not mutates) the array so `useSyncExternalStore`'s identity check stays cheap and the widget only re-renders on real changes.
- `installConsoleCapture()` patches `console.info`/`warn`/`error` **in addition** to passing through to the originals (the patched method calls `capture(level, args)` then `original(...args)`), so existing `console.error`-based guards (the e2e `console-guard` listens at the browser level via `page.on('console', ...)`) keep working unchanged. It is idempotent (`installed` guard) and a no-op when `console` is unavailable (SSR / node). `useLogger()` auto-installs on first call — simply mounting the widget is enough; no `App.tsx` or loop wiring. Public surface: `useLogger()`, `clearLogs()`, `getLogs()`, `subscribe()`, `installConsoleCapture()`, plus `_resetLoggerForTests()` (test-only).
- `apps/simulator/src/components/widgets/logs-widget.tsx` — replaces the Phase-2 "No logs yet" stub. It reads `useLogger()` and renders a `font-mono` scroll box (`data-testid="logs-view"`) like the reference Utils tab's "System Logs" panel: each line is `HH:MM:SS LEVEL text` with level-coloured text (`text-muted-foreground` for info, `text-amber-500` for warn, `text-red-500` for error). An auto-scroll effect pins to the bottom on every new entry **unless** the user has scrolled up to read older lines (`onScroll` toggles `pinned`). A `Clear` button (`data-testid="logs-clear"`, disabled when empty) calls `clearLogs()`. The `WidgetCard` `actions` slot hosts the Clear button so it sits in the header next to the title. `No logs yet` placeholder is shown when the buffer is empty. Non-string args are quoted (`JSON.stringify` for objects, `String` for primitives).
- `apps/simulator/src/lib/logger.test.ts` (new) — 8 node-only unit tests covering the non-React surface: capture of info/warn/error lines (with unique monotonic ids), idempotent install, pass-through to the original handler, non-string arg stringification, the `MAX_LOGS=200` ring-buffer cap (asserts the oldest 50 are dropped and the newest surviving entry is "line 50"), `clearLogs` notifies subscribers, `subscribe` fires per capture and is unsubscribable, and `useLogger` is a no-op-safe import. All `console.*` calls are funneled through a single `emit()` helper that carries the one `// biome-ignore lint/suspicious/noConsole:` suppression site, keeping the test file lint-clean.
- `apps/simulator/e2e/logs-widget.spec.ts` (new) — 5 e2e tests driving the real in-page capture→render→clear flow. The shared console guard flags `console.error`, so the spec only injects `info`/`warn` lines (`phase5 hello`, `phase5 careful`, etc.) and asserts they surface in `logs-view` with the right `INFO`/`WARN` level tag, accumulate in DOM order, and that the `logs-clear` button empties the buffer back to the `No logs yet` placeholder. (The simulator emits a `THREE.Clock` deprecation `console.warn` at startup, so the empty-placeholder test clears first — which double-proves the Clear path.)
- **Dead-code cleanup (option b):** the six legacy HUD files are **deleted** — `sensor-hud.tsx`, `map-hud.tsx`, `navigation-hud.tsx`, `teleop-hud.tsx`, `localization-hud.tsx`, `debug-overlay.tsx`. The widget files had already inlined their bodies as far back as Phase 2; the old files were dead (no `import` of them survived anywhere in `src/` or `e2e/` — the only references left were comments mentioning "the old `SensorHud`" etc.). Those widget-file comments are rewritten to read as standalone ("The lidar sliders + …", not "Wraps the old `SensorHud`") so there is no confusing indirection layer pointing at deleted files. `App.tsx` had no dead imports of the old HUDs since Phase 2; nothing else needed un-exporting. Confirmed: `rg` for `from '…/(sensor-hud|map-hud|navigation-hud|teleop-hud|localization-hud|debug-overlay)'` across `src/`+`e2e/` returns no matches.
- Verified: `pnpm typecheck` clean, `pnpm test` 90/90 (was 82; +8 logger unit tests), `pnpm lint` clean (0 warnings / 0 errors), `pnpm check` clean, `pnpm build` clean, `pnpm test:e2e` 63/63 (was 58; +5 `logs-widget.spec.ts`). The console-guard suite stays green — the pass-through patch preserves the browser-level `console.error` interception.
...
- **Unit (vitest):** `store` panel slice —
  ...
  - `logger.test.ts` (new, Phase 5) — ring-buffer logger (node-only, no jsdom): idempotent `installConsoleCapture`, capture of `console.info`/`warn`/`error` into the buffer with unique monotonic ids, pass-through to the original handler, non-string arg stringification (objects via `JSON.stringify`), the `MAX_LOGS=200` cap (oldest dropped), `clearLogs` empties + notifies subscribers, `subscribe` fires per capture and is unsubscribable, and `useLogger` is a no-op-safe import.
- **E2E (playwright):**
  ...
  - `logs-widget.spec.ts` (new, Phase 5) — the Utils-tab System Logs widget: `data-testid="logs-view"` shows `No logs yet` when empty; injected `console.info`/`console.warn` lines (the shared guard flags `console.error`, so only `info`/`warn` are injected here) surface with the right `INFO`/`WARN` level tag and accumulate in DOM order; `data-testid="logs-clear"` empties the buffer back to the placeholder and is disabled while empty.
...
**Phase 6 implementation notes (landed):**
- The unit layer (`apps/simulator/src/store.test.ts`, 90/90 with the Phase-5 logger suite) already locks every Phase-6 contract: `togglePanel` is the sole flipper of `panelOpen` (independent tests that closing the panel first then dragging / dropping / cancelling / un-docking leaves `panelOpen` untouched); `setTab` switches tabs without touching `panelOpen`; `startDragWidget`/`dropPoppedWidget`/`cancelDragWidget`/`undockWidget` match the Phase-4 signatures (no panel coupling); `dropPoppedWidget` singleton move semantics (re-drop replaces the existing copy at the new edge); `isWidgetPopped`; and the theme slice (`toggleTheme` flips + persists, `setTheme` sets + persists, initial fallback falls through `localStorage` → `matchMedia('(prefers-color-scheme: dark)')` → `dark`, with the localStorage + matchMedia mocks in the test file). `apps/simulator/src/lib/logger.test.ts` covers the ring-buffer logger (idempotent install, info/warn/error capture with monotonic ids, pass-through to the original handler, non-string arg stringification, the 200-line cap, `clearLogs` notification, `subscribe`/unsubscribe, `useLogger` import-safety).
- The e2e layer is the full Phase-6 matrix, all green (`pnpm test:e2e` → **63/63**):
  - `right-panel.spec.ts` — panel visible by default with all 5 tabs; collapse toggle slides `translate-x-full` so the closed panel's `x + width > innerWidth` (Phase 4a geometry assertion); reopening restores it; each tab selectable; the 5th (`utils`) tab is off the strip until horizontal-scroll (`scrollIntoView({inline:'center'})`); keyboard Arrow / Home / End nav moves between tabs and wraps.
  - `widgets-drag.spec.ts` — real `page.mouse` dnd-kit drive (nudge past the 8px activation threshold → wait for `drop-zones` → move to edge → wait for `drop-zone-${edge}[data-over=true]` → `mouse.up`); dragstart **does not close the panel** (`data-open` stays `true`) but shows zones; drop moves the lidar widget and the panel handle count drops by one (move, not duplicate); the popped close button moves it back; the popped re-dock handle moves it to a new edge; re-dropping never duplicates; the camera-feed widget pops out **carrying its live viewport** and closing returns the feed.
  - `viewport-edge-docks.spec.ts` (Phase 4c) — pairwise clipped bounding boxes per edge are disjoint; `top`+`left` strips never intersect; dropping enough cards on one edge forces the strip's `scrollHeight > clientHeight`; re-docking a widget from `top` to `right` moves it between strips (top count drops, right count grows).
  - `theme-toggle.spec.ts` (Phase 4d) — app loads in dark mode by default (`<html>` carries `dark`); clicking `theme-toggle` flips both `document.documentElement.classList` and the button's `aria-pressed`; the choice is persisted to `localStorage['robotics-lab.theme']` and survives `page.reload()`.
  - `logs-widget.spec.ts` (Phase 5) — `logs-view` shows `No logs yet` when empty; injected `console.info` lines surface with the `INFO` tag, `console.warn` lines with the `WARN` tag, and both accumulate in DOM order; `logs-clear` empties the buffer back to the placeholder and is disabled while empty. **Fix applied in this pass:** three of the five tests had a leftover stub `page.evaluate(() => {})` (an **empty** arrow) where the inject-an-actual-`console.info`/`warn` call should have been — so the asserts were waiting for text that was never emitted. Each is now wired to a real `console.info`/`console.warn` payload matching its expected text (`phase5 hello`, `first`, `phase5 goodbye`); the suite went 2/5 → 5/5.
  - The pre-existing control specs (`smoke`, `sensors`, `navigation`, `teleoperation`, `localization`, `occupancy-grid`, `console`) had already been updated in Phase 2 to switch to the right tab before clicking controls (`activateTab(page, 'utils'|'nav'|'teleop'|'map')`), keeping all `data-testid`s on the underlying controls identical, so their selectors target the same controls now living inside the right panel tabs.
- Verified: `pnpm typecheck` clean, `pnpm test` 90/90, `pnpm lint` + `pnpm check` clean, `pnpm build` clean, `pnpm test:e2e` 63/63.
